...
declare_signals! {
    ...
    Signal::Poll => libc::SIGIO, //TODO: libc::SIGPOLL,
...
fn clock_seconds(clock: libc::clockid_t) -> u64 {
    unsafe {
        let mut up: libc::timespec = std::mem::zeroed();
        if libc::clock_gettime(clock, &mut up) == 0 {
            up.tv_sec as u64
        } else {
            sysinfo_debug!("clock_gettime failed: time cannot be retrieved...");
            0
        }
    }
}
...
fn boot_time() -> u64 {
    let uptime = uptime();
    clock_seconds(libc::CLOCK_REALTIME) - uptime
}
